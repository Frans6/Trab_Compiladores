...
static void print_value(Value v) {
    switch (v.type) {
        ...
        case VAL_BOOL:   printf(v.data.b ? "true\n" : "false\n"); break;
...
Value eval_node(ASTNode* node) {
    if (!node) { Value v = {VAL_INT, .data.i = 0}; return v; }
    switch (node->type) {
        case AST_INT: {
            Value v = { VAL_INT,   .data.i = node->data.int_val }; return v;
        }
        case AST_FLOAT: {
            Value v = { VAL_FLOAT, .data.f = node->data.float_val }; return v;
        }
        case AST_STRING: {
            Value v = { VAL_STRING, .data.s = node->data.str_val }; return v;
        }
        case AST_BOOL: {
            Value v = { VAL_BOOL,  .data.b = node->data.bool_val }; return v;
        }
        case AST_VAR: {
        ...
        case AST_BINOP: {
            ...
            fprintf(stderr, "Erro: binop tipos não suportados\n");
            exit(1);
        }
        case AST_ASSIGN: {
        ...
        default: {
            fprintf(stderr, "Erro: nó inválido em eval_node: %d\n", node->type);
            exit(1);
        }
    }
}
...
void exec_ast(ASTNode* node) {
    ...
    switch (node->type) {
        ...
        case AST_IF: {
            ...
            if (c.type != VAL_BOOL) {
                fprintf(stderr, "Erro: condicional não booleano\n");
                exit(1);
            }
            if (c.data.b)
